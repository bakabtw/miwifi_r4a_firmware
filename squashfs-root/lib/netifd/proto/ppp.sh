...
proto_pppoe_init_config() {
	...
	proto_config_add_string "special"
	lasterror=1
}
...
proto_pppoe_setup() {
	...
	json_get_var special special

    #adjust holdoff timeout by last pppoe state
    local last_state
    ...
    #check if special dial needed
	local f_pppoe_special="/tmp/state/pppoe_special"
    if [ "$special" = "1" ]; then
        local specdial
        [ -f "$f_pppoe_special" ] && specdial=$(cat "$f_pppoe_special")
        ...
        #clear last pppoe special if exist
        [ -f "$f_pppoe_special" ] && rm -rf $f_pppoe_special

        #pppoe dial without specdial
        ppp_generic_setup "$config" \
            plugin rp-pppoe.so \
            ${ac:+rp_pppoe_ac "$ac"} \
            ${service:+rp_pppoe_service "$service"} \
...
proto_pppoe_teardown() {
    ...
    #save pppoe special dial if switch ON
	local f_pppoe_special="/tmp/state/pppoe_special"
    if [ "$special" = "1" ]; then
        case "$ERROR" in
            11|19)
            local spec_val
            for i in 0 1 6 7; do
                spec_val=$(cat "$f_pppoe_special" 2>/dev/null)
                if [ -n "$spec_val" -a "$spec_val" = "$i" ]; then
                    echo -n > "$f_pppoe_special"
                    continue
                fi
                if [ -z "$spec_val" ]; then
                    echo $i > "$f_pppoe_special"
                    break
                fi
            done
            ;;
            *)
            [ -f "$f_pppoe_special" ] && rm -rf $f_pppoe_special 2>/dev/null
            ;;
        esac
    else
        [ -f "$f_pppoe_special" ] && rm -rf $f_pppoe_special 2>/dev/null
